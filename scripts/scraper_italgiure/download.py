"""Download e validazione PDF Italgiure."""
...
def looks_like_captcha(data: bytes) -> bool:
    ...
    return any(i in head for i in indicators)
...
def verify_pdf(data: bytes, *, min_bytes: int, max_bytes: int) -> list[str]:
    ...
    if looks_like_captcha(data) and not is_pdf_bytes(data):
        errors.append("pagina captcha/blocco invece di PDF")
...
def _replace_with_retry(src: Path, dest: Path, *, attempts: int = 8) -> None:
    """os.replace su Windows fallisce con WinError 32 se AV/Explorer tiene il file."""
    ...
    for i in range(attempts):
        try:
            ...
            last = exc
            # 32 = sharing violation, 5 = access denied (a volte antivirus)
...
def save_pdf_atomic(dest: Path, data: bytes, *, overwrite: bool = False) -> dict:
    ...
    # tmp univoco: evita collisioni se girano due scraper / AV sul .tmp fisso
    tmp = dest.with_name(f"{dest.name}.{os.getpid()}.{time.time_ns()}.tmp")
    try:
        tmp.write_bytes(data)
        # flush su disco prima del rename (riduce lock AV mid-replace)
