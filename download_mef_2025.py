...
def load_server_keys(year: int, cache_dir: Path) -> Tuple[set[str], str]:
    ...
    Unisce anche gli indici locali da D: (mef_skip_from_d_*.txt) se presenti.
    """
    sources: list[str] = []
    keys: set[str] = set()
    ...
    if year:
        ...
        if year_file.exists():
            keys |= _read_nome_keys(year_file)
            sources.append(str(year_file))

    if not keys:
        keys_path = Path(cache_dir) / "nomi_base.txt"
        if keys_path.exists():
            keys |= _read_nome_keys(keys_path)
            sources.append(str(keys_path))

    if not keys:
        full_list = CACHE_NOMI_BASE_DIR / "cache_nomi_base.txt"
        if full_list.exists():
            keys |= _read_nome_keys(full_list)
            sources.append(str(full_list))

    # Extra: archivio locale D: (generato da integra_d_drive_skip.py)
    root = Path(__file__).resolve().parent
    for extra in (
        root / "mef_skip_from_d_all.txt",
        root / f"mef_skip_from_d_{year}.txt" if year else None,
    ):
        if extra is None:
            continue
        extra_keys = _read_nome_keys(extra)
        if extra_keys:
            keys |= extra_keys
